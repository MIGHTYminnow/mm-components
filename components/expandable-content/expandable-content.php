...
add_action( 'register_shortcode_ui', 'mm_components_mm_expandable_content_shortcode_ui' );
/**
 * Register UI for Shortcake.
 *
 * @since  1.0.0
*/
function mm_components_mm_expandable_content_shortcode_ui() {

	if ( ! function_exists( 'shortcode_ui_register_for_shortcode' ) ) {
		return;
	}

	$link_styles           = mm_get_link_styles( 'mm-expandable-content' );
	$button_styles         = mm_get_button_styles( 'mm-expandable-content' );
	$button_border_weights = mm_get_button_border_weights( 'mm-expandable-content' );
	$button_corner_styles  = mm_get_button_corner_styles( 'mm-expandable-content' );
	$colors                = mm_get_colors( 'mm-expandable-content' );
	$text_alignments       = mm_get_text_alignment( 'mm-expandable-content' );

	shortcode_ui_register_for_shortcode(
		'mm_expandable_content',
		array(
			'label'         => esc_html__( 'Mm Expandable Content', 'mm-components' ),
			'listItemImage' => MM_COMPONENTS_ASSETS_URL . 'component-icon.png',
			'attrs'         => array(
				array(
					'label'       => esc_html__( 'Button or Link?', 'mm-components' ),
					'description' => esc_html__( 'Should the trigger be a button or a link?', 'mm-components' ),
					'attr'        => 'link_style',
					'type'        => 'select',
					'options'     => $link_styles,
				),
				...
				array(
					'label'       => esc_html__( 'Button/Link Text', 'mm-components' ),
					'description' => esc_html__( 'The text for the button/link', 'mm-components' ),
					'attr'        => 'link_text',
					'type'        => 'text',
				),
				array(
					'label'       => esc_html__( 'Button/Link Alignment', 'mm-components' ),
					'attr'        => 'link_alignment',
					'type'        => 'select',
					'options'     => $text_alignments,
				),
				array(
					'label'       => esc_html__( 'Fade in?', 'mm-button' ),
					'attr'        => 'fade',
					'type'        => 'checkbox',
				),
				array(
					'label'       => esc_html__( 'Extra class name', 'mm-components' ),
					'description' => esc_html__( 'Style particular content element differently - add a class name and refer to it in custom CSS.', 'mm-components' ),
					'attr'        => 'class',
					'type'        => 'text',
				),
			),
